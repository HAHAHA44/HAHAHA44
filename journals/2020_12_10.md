...
### 但是，比如这两天就遇到了一个问题，在 iOS Safari 上有一个 player 相关的bug，设置 Track 必须要和 play 操作的间隔大于 2s。这个时候就需要对 player 这个对象做出一些改动，来确保 play 和 updateTrack 的间隔。要注意的一点是，我们是不是可以直接复用已有的 video DOM。SDK之前的设计是没有复用 video DOM，每次更新 Track 的时候都会去新建一个 videoDOM 来替换掉文档树上老的 videoDOM。这个设计可能是出于保证功能的角度，但是实际上 videoDOM 是可以复用的，只需要在更新 srcObject 之后调用 video.load() 方法。
